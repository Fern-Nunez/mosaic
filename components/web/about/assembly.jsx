...
/**
 * Scroll-driven assembly: apart and colourless at progress 0, connected and
 * in colour at 1.
 *
 * Tuned at /test/scroll, where the same scene is driven by query parameters
 * (?aspect=&flatten=&seed=). Copy new values across once you like them.
 *
 ...
 * back, so desktop settings leave a thin strip. zoom and flatten are the two
 * that fix it without touching the layout — overriding aspect or count would
 * give a different tile pattern on mobile than on desktop.
 */
const MOBILE = { zoom: 1.2, flatten: 0.75, offsetY: -0.3, dpr: 1.25, samples: 10, scrub: 0.3};
...
export default function Assembly({
  ...
  offsetX = 0,
  offsetY = -0.06,
  /** Overrides applied below . Pass {} to disable them. */
  mobile = MOBILE,
  breakpoint = 768,
}) {
  const narrow = useIsNarrow(breakpoint);
  const v = narrow
    ? { zoom, flatten, offsetX, offsetY, ...mobile }
    : { zoom, flatten, offsetX, offsetY };
